...
// --- Blog Content JSON to HTML Conversion ---

/**
 * Extended Image with width and alignment attributes for blog content
 */
...
/**
 * Extensions used for blog content (shared between editor and HTML generation)
 * Note: ImageUploadNode and Selection are excluded as they're client-side only
 */
...
/**
 * Converts TipTap JSON content to HTML string for rendering
...
export function convertJsonToHtml(jsonString: string): string {
  try {
    ...
    return generateHTML(json, blogContentExtensions)
  } catch (error) {
